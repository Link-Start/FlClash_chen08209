...
/// Opens the HTML comment that carries the translated entries to the app. The
  ...
  for (final version in changelog.versions) {
    ...
    for (final group in version.groups) {
      ...
      for (final entry in group.entries) {
        buffer.writeln(
          '- ${_prefix(entry)}${entry.textFor(changelogFallbackLocale)} '
          '(${entry.id})',
        );
      }
      buffer.writeln();
...
String renderTelegram(ChangelogVersion version, {required String moreUrl}) {
  ...
  for (final group in version.groups) {
    buffer.writeln('*${group.type.title}*');
    for (final entry in group.entries) {
      buffer.writeln('• ${entry.textFor(changelogFallbackLocale)}');
    }
    buffer.writeln();
  }
  ...
  return '${cut.substring(0, lastBreak < 0 ? cut.length : lastBreak)}\n\n…\n$moreUrl';
}
