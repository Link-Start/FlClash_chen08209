...
void main() {
  ...
  group('sortByChar', () {
    test('equal strings return 0', () {
      expect(sortByChar('abc', 'abc'), 0);
    });

    test('empty first returns -1', () {
      expect(sortByChar('', 'a'), -1);
    });
    ...
    test('case insensitive comparison', () {
      expect(sortByChar('a', 'B'), lessThan(0));
      expect(sortByChar('B', 'a'), greaterThan(0));
    });
  });
  ...
  group('fastHash', () {
    test('produces consistent hash', () {
      final hash1 = fastHash('hello');
      final hash2 = fastHash('hello');
      expect(hash1, hash2);
    });

    test('different input produces different hash', () {
      expect(fastHash('hello'), isNot(equals(fastHash('world'))));
    });

    test('returns an integer', () {
      final hash = fastHash('test');
      expect(hash, isA<int>());
    });
  });
  ...
  group('getTimeDifference', () {
    test('formats a sub-minute difference', () {
      final value = getTimeDifference(
        DateTime.now().subtract(const Duration(seconds: 5)),
      );
      expect(value, matches(RegExp(r'^\d{2}:\d{2}:\d{2}$')));
      expect(value.startsWith('00:00:'), isTrue);
    });

    test('reports elapsed hours and cumulative minutes', () {
      final value = getTimeDifference(
        DateTime.now().subtract(const Duration(hours: 2, minutes: 3)),
      );
      expect(value.split(':').first, '02');
    });
  });
  ...
  group('getMaterialColorShades', () {
    test('returns the full material shade ramp', () {
      final shades = getMaterialColorShades(const Color(0xFF2196F3));
      expect(shades.length, 11);
      expect(shades.toSet().length, greaterThan(1));
    });

    test('is deterministic for the same seed color', () {
      const seed = Color(0xFF4CAF50);
      expect(getMaterialColorShades(seed), getMaterialColorShades(seed));
    });
  });
}
