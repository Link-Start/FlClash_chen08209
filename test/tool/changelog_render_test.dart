...
ChangelogGroup entryGroup(
  ...
  entries: [
    for (final (id, scope, text) in entries)
      ChangelogEntry(id: id, scope: scope, text: {'en': text}),
  ],
);
...
void main() {
  ...
  group('renderRelease', () {
    ...
    test('carries the translated payload the app reads', () {
      ...
      expect(
        decoded.versions.single.groups.single.entries.single.textFor('zh-CN'),
        '覆写脚本',
      );
    ...
    test('escapes > so an entry cannot close the comment early', () {
      ...
      expect(
        decodeChangelog(
          payload,
        ).versions.single.groups.single.entries.single.textFor('en'),
        'Handle a --> sequence',
      );
    });
  });
