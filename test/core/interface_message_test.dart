...
// Both transports resolve a timed-out call to null: the desktop RPC client
// catches TimeoutException and the Android bridge passes onTimeout: () => null.
...
void main() {
  ...
  group('an unanswered call is not reported as an applied change', () {
    cases.forEach((name, invoke) {
      test('$name surfaces the silence instead of an empty message', () {
        // An empty message is this protocol's "applied, no error", so coercing
        // a missing answer to '' told callers the change took effect.
        expect(
          invoke(),
          throwsA(
  ...
  test('calls that legitimately degrade are left alone', () async {
    ...
    expect(await core.getCountryCode('1.1.1.1'), isEmpty);
