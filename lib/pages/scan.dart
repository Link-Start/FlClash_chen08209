...
class _ScanPageState extends ConsumerState<ScanPage>
  ...
  // `MobileScanner` starts and stops any `autoStart` controller from its own
  // initState, but only observes the app lifecycle for controllers it created
  // itself. This page supplies the controller and drives the lifecycle below,
  // so autoStart would just race our own `start()` and throw
  // `controllerInitializing` from whichever call loses.
  ...
  void _handleBarcode(BarcodeCapture barcodeCapture) {
    // The subscription is cancelled in dispose, but a capture already queued on
    // the stream still arrives afterwards. Popping then targets a dead route.
    if (!mounted) {
      return;
    }
