  ...
  Future<AuthorizeCode> registerService() async {
    ...
    switch (readiness) {
      ...
      case WindowsHelperReadiness.manifestMissing:
        ...
        dialogs.showNotifier(currentAppLocalizations.helperCorruptTip);
        return AuthorizeCode.error;
...
class MacOS implements SystemDnsPort {
  ...
  /// Reads the service name that `-listnetworkserviceorder` reports for
  /// [device].
  ///
  /// Everything after the `(n)` index belongs to the name, spaces included.
  /// Splitting on whitespace and taking one field truncates `Thunderbolt
  /// Bridge` and `iPhone USB` down to their first word, which no later
  /// `networksetup` call then accepts.
  @visibleForTesting
