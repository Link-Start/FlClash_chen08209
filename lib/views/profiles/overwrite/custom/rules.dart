...
class _CustomRulesViewState extends ConsumerState<CustomRulesView> {
  ...
  Future<bool> _handleDelete(Set<dynamic> selectedRules) async {
    ...
    // The confirmation above is open for as long as the user leaves it open, so
    // the sheet holding this view can be gone by the time it resolves.
    if (!mounted) {
      return false;
    }
    ref
      ...
      PagedSheetRoute(
        builder: (context) => Consumer(
          builder: (_, ref, _) {
            ...
            final proxies = proxiesAndGroups.proxies;
            final proxyGroups = proxiesAndGroups.proxyGroups;
            return OverwriteSelectionSheet<String>(
