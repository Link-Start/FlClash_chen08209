...
class CoreAction extends _$CoreAction {
  ...
  Future<void> startCore() async {
    ...
    try {
      await _core.start();
      // Published only once init has succeeded: announcing connected first made
      // an init failure show up as connecting -> connected -> disconnected.
      await initCore();
      ref.read(coreStatusProvider.notifier).value = CoreStatus.connected;
    } catch (error) {
      ref.read(coreStatusProvider.notifier).value = CoreStatus.disconnected;
      dialogs.showNotifier(error.toString());
    }
  }
