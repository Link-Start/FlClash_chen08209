...
class BackupAction extends _$BackupAction {
  ...
  /// Writes [data] into the database and, unless [option] limits the restore to
  /// profiles, into the settings providers.
  ///
  /// The settings are parsed before the database is touched. `setAll` deletes
  /// every profile the backup does not carry, so a parse that failed afterwards
  /// would leave a replaced profile set behind a `currentProfileId` pointing at
